...
const ListedVehicles = () => {
  return (
    <Box>
      ...
      <a href='#'><Typography  sx={{ color: 'primary.main',textAlign:'right' }}>
        <u>View all listed Vehicles</u>
      </Typography></a>
      <Carousel>
        {chunkedAdData.map((chunk, index) => (
          <div key={index} style={{ display: 'flex', flexDirection: 'row', marginTop: '20px', justifyContent: 'center' }}>
            {chunk.map((ad, adIndex) => (
              <Card key={adIndex} sx={{ width: 300, height: 330, marginRight: 2 }}>
                <img src={ad.image} alt={ad.title} style={{ maxWidth: '100%', maxHeight: '100%' }} />
                <CardContent>
                  <Typography><b>{ad.title}</b></Typography>
                  <Typography sx={{color:"success.main"}}>{ad.price}</Typography>
                  <Typography sx={{color:"primary.light"}}>{ad.location}</Typography>
                </CardContent>
              </Card>
            ))}
          </div>
